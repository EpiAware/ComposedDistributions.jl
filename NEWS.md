...
- **breaking:** `params_table(d)` is removed. `composed_to_table(d)` is now
  ...
  This is a breaking change folded into the same unregistered 0.2.0 window
  as the rest of the #227 table work, not a deprecation cycle.
- **feature:** `composed_to_table(d)` returns the full node/attribute/
  ...
  parameter — from one shared pre-order walk. A composed
  ...
  `composed_to_table` as the explicit way to copy rows,
