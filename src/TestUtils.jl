...
# and drops it with no error: `params_table` shows the parameter as fixed,
# `has_uncertain` is false, a tied leaf stops deduplicating. This check builds
...
- the attached `prior` on `param` survives into `wrap(...)`'s
  [`uncertain_specs`](@ref) and `params_table`'s `prior` column, and
...
function test_leaf_protocol_completeness(wrap::Function;
    ...
    return @testset "leaf-protocol completeness: $name" begin
        ...
        # into params_table's prior column and has_uncertain.
        ...
        @testset "uncertain_specs (prior survives the wrapper)" begin
            ...
            # `params_table` is a composer-tree method; wrap the leaf in a
            # one-branch tree to reach it, matching how a leaf-protocol
            # question is normally asked (a bare leaf never appears alone at
            # the top of a fitted tree).
            tbl = params_table(compose((leaf_protocol_leaf = wrapped,)))
            rows = findall(==(param), tbl.param)
            @test !isempty(rows)
