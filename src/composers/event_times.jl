...
# A `Resolve`/`Compete`/`Choose` is a single value slot (the outcome that fired),
# so it is one increment from its parent like any leaf. Positions are unitless
# distance from the origin; calendar mapping belongs to the model layer (#269).

# Promote the record's value types to one element type (AD `Dual`s flow through).
...
`event_increments(d, event_times(d, rec)) == rec`. Pass a `Vector` of records
for a batch.
