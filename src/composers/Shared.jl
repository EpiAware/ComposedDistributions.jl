...
the SAME free parameter by the prior/params interface: [`composed_params`](@ref)
...
- [`composed_params`](@ref), [`update`](@ref): dedup occurrences by tag.
"
...
A `shared(:tag, dist)` leaf carries a tag that ties every occurrence to one set
of parameters. The tag survives wrapper leaves (a `Truncated`, and the
...
`nothing`. [`composed_params`](@ref) uses the tag as a leaf's edge and inventories
...
# tag consumer (`composed_params`, `build_priors`, `update`,
...
consumer ([`composed_params`](@ref), [`build_priors`](@ref), [`update`](@ref), a
...
[`composed_params`](@ref)'s `edge` column), or a tuple of edge names from the root.
