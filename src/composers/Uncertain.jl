...
# A literature-reported Gamma delay with an uncertain shape (alpha).
...
# Nested: the shape (alpha) prior's location is itself uncertain.
...
# tree with one or more of an already-built tree's free parameters promoted
# to uncertain. They dispatch on `AbstractComposedDistribution` (not a
...
#
# The underlying mechanism is the same as `update`'s existing merge mode
...
# using `update`'s auto-detection; a caller who already knows this call
# promotes something reaches for `uncertain` instead, and gets a loud error
# if it turns out to promote nothing.
