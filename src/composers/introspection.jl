...
[`params_table`](@ref)'s `prior` column. Without a forwarding method the
...
`params_table`, `uncertain` and the flat codec work in. A moment-parameterised
wrapper naming a mean and a standard deviation, rather than a shape and a
scale, is the motivating case.
...
[`params_table`](@ref) and [`uncertain`](@ref) key on.
...
`Tables.istable`, `Tables.columns`, `Tables.getcolumn` and `DataFrame(tbl)`
all consume it unchanged; only its display is customised.

See also: [`params_table`](@ref), [`update`](@ref)`(tree, table)`.
"
...
- `support`: the `(minimum, maximum)` variate support of that edge's
  ...
  `maximum`/`support`).
- `prior`: the attached spec of an [`uncertain`](@ref) parameter — a
  distribution, a [`pool`](@ref)`(...)` group, or the [`no_prior`](@ref)`()`
  marker (free, no prior yet) — or `nothing` for a fixed parameter.

Read parameter structure off the rows of this table instead of hand-matching
names, and write priors back onto it (edit `prior`, then
[`update`](@ref)`(tree, table)`) instead of hand-matching the tree. Built from
[`params`](@ref) (nested, name-keyed values) plus the edge distributions'
support.
...
# each row's `prior` entry (`nothing` for a fully fixed parameter).
...
:stick_{K-1}` in [`params_table`](@ref) and a fitted chain), each a `Beta` in
(0, 1), so every draw lands on the probability simplex and the gradient is
well-defined. The probabilities are recovered from any draw: a strict `update`
from the stick coordinates (as read back from a chain) collapses the node to
concrete probabilities summing to one (read them with `Distributions.probs`).
`uncertain(tree)` (bare) marks the simplex `no_prior()` per `Resolve`, no
prior chosen.
...
# (bare `uncertain(tree)`, below): both walk a `params_table`-shaped set of
# rows and assemble the identical nested-`NamedTuple` shape `update`/
