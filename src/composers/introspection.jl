...
Deprecated alias for [`composed_params`](@ref) (#227).

`params_table` read as extending `Distributions`' `params`, which it does
not (it is CD's own tree-shaped Tables.jl source, not comparable to a plain
`params` tuple), and the `_table` suffix repeated what `composed_` already
signals about the shape. Kept exported for one release so the deprecation
warning fires unqualified too; use [`composed_params`](@ref) instead.

Written out by hand rather than via `@deprecate`: that macro always expands
to a multi-statement `:toplevel` block (even with its `export` flag set to
`false`), which a preceding docstring cannot attach to, silently leaving the
alias undocumented.

# Arguments
- `d`: the composed distribution to inventory; forwarded unchanged to
  [`composed_params`](@ref).
...
tree = compose((onset_admit = Gamma(2.0, 1.0),))
params_table(tree)  # deprecated; prefer composed_params(tree)
```
"
function params_table(d)
    Base.depwarn(
        "`params_table(d)` is deprecated, use `composed_params(d)` instead.",
        :params_table)
    return composed_params(d)
end
...
function _update_branch_probs(c::Resolve, delays, params::NamedTuple,
    ...
    if merge
        ...
        # is what makes `update(tree, composed_params(tree))` round-trip a
