...
# `params_table`; `leaf_detail_lines` routes a leaf's `inspect` rendering;
...
# these silently drops an attached spec on a wrapped leaf (`params_table`
# then treats it as fixed). See `docs/src/developer/leaf-protocol.md`.
...
# before `params_table` walks it per gradient evaluation.
...
# `params_table`, `uncertain` and the flat codec work in. A leaf
