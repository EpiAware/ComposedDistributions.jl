...
Mark the parameters to estimate by building a leaf with [`uncertain`](@ref), or promote a leaf/subtree/whole tree already in place with [`uncertain`](@ref)`(tree, ...)` / `uncertain(tree)`, then either package the tree and data into a log-density with [`as_logdensity`](@ref), or reach for `DistributionsInference.jl` — `as_logdensity`/`as_turing` there give any `LogDensityProblems` consumer or a `DynamicPPL`/Turing.jl model, built on the same codec.
