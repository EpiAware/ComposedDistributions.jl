...
- [`Uncertain`](@ref) is the richer latent leaf: it also carries each parameter's **prior** (so [`composed_to_table`](@ref) rides it on the `prior` column), draws the marginal with `rand`, and collapses via [`update`](@ref).
