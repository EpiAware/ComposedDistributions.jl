...
Extending only `free_leaf` and `rewrap_leaf` is enough for a fixed-structure wrapper; a wrapper that attaches priors or owns extra parameters extends the rest so those reach `composed_params` and `build_priors`.
...
Most wrappers carry only fixed structure, so their extra-parameter map is empty and `composed_params` shows just the inner delay's rows.
...
6. Register the wrapper with `register_leaf_wrapper!` (see below) so the generated flat-vector codec (`flat_dimension`/`unflatten`/`flatten`/`reconstruct`) sees through it too — steps 1-5 alone cover `composed_params`/`build_priors`, but not the generated codec.
