...
`free_leaf`/`rewrap_leaf` are the two mandatory hooks; everything else is optional, with a default that is correct for a wrapper that carries *nothing extra* through.
...
|---|---|---|
| [`free_leaf`](@ref) | peel to the innermost free delay | none — mandatory |
